...
### Architectural notes
- Hover outline now uses theme properties `button_hover_outline_color` / `button_hover_outline_width` (extracted from hardcoded values).
...
## Asset Scaling: 64→128px + Idle Bob + Battle Background (2026-03-11)

### Scaling strategy (`assetgen/battle_sprites.py`, `assetgen/battle_tiles.py`)
All battle assets doubled from 64×64 to 128×128 (sprites) and 72×72 to 144×144 (select ring).

**Approach:** Content-scale factor `_SCALE = 2` multiplied into the `_s()` / `_si()` helpers alongside the existing `_SS = 4` supersampling factor. This means:
- Coordinates authored at 1× (originally targeting 64px output) are multiplied by `_SS * _SCALE = 8` in supersampled space
- Final LANCZOS downsample produces 128×128 instead of 64×64
- `CX, CY` updated from 32 → 64 to match

Both `battle_sprites.py` and `battle_tiles.py` use the same pattern. Health bars scaled from 40×6 → 96×10.

### Select ring coordinate space note
The select ring's `_RING_BBOX_1X = (8, 32, 134, 110)` is authored directly in 144×144 output space (already doubled). The bbox is scaled by `_SS` only (not `_SS * _SCALE`). However, `_s()` / `_si()` calls for padding/widths within `_draw_select_ring()` use `_SS * _SCALE`. This means the ring's glow radius, outline widths, and specular positions are effectively doubled relative to the bbox. The ring renders and looks acceptable but the coordinate spaces are mixed. **Not blocking** — cosmetic parameters tuned to current output.

### Robustness fix: `_valid_bbox()` guard
New helper `_valid_bbox()` guards `radial_gradient` and `Image.crop` calls that could fail with zero-area bounding boxes. Applied to shield gem halo and skeleton eye glow rendering. Prevents edge-case crashes when scaled coordinates clamp to the same pixel.

### Battle demo changes (`examples/battle_vignette/`)
1. **`battle_unit.py`** — `SPRITE_SIZE = 128`, health bar geometry scaled (96×10, -12 offset), Tailwind color tokens for HP bar (Emerald-500 / Rose-500). New idle bob: sine wave at 2 Hz, 3px amplitude, skipped during composable actions via `sprite._current_action` check.

2. **`battle_demo.py`** — Background color updated to Slate-900. Full-screen textured background sprite (`tiles/battle_bg`) at `RenderLayer.BACKGROUND`. New `update()` override calls `unit.update(dt)` for all living units (idle bob).

3. **`battle_grid.py`** — `TILE_SIZE = 128`. Highlight colours updated to Tailwind tokens (Sky-400, Rose-500).

### New asset: `battle_bg.png` (1920×1080)
Generated by `make_battle_bg()` in `battle_tiles.py`. Slate-900 base with radial vignette, noise grain, and subtle floor band. No supersampling (direct render for efficiency at full resolution).

### Bug fixed: `test_battle_tiles.py` expected sizes
The visual test script `assetgen/test_battle_tiles.py` still checked for 64×64 tiles and 40×6 health bars. Updated to 128×128 and 96×10. Tile sheet layout also updated to use 128px grid.
