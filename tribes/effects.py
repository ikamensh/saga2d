"""Transient visual effects for Tribes: floating text, pulses, hit reactions, banners.

The scene creates effects in response to model events and keeps them in an
:class:`Effects` list it updates and draws every frame.  World-space effects
take a world position (use :func:`tribes.view.tile_center`), so they work
under any tile projection.  Sprite effects (flash, knockback, dissolve) only
touch the sprite they are given and restore it when they finish.

Sound goes through :func:`play_sound`; set :data:`sound_hook` to
``SoundBank.play`` to hear it.
"""
...
class TilePulse(Effect):
    """Rings of *color* expanding from a world position: capture, level up."""

    def __init__(
        self, position: tuple[float, float], color: Color, *,
        radius: tuple[float, float] = (14.0, 58.0), rings: int = 2, duration: float = 0.9, delay: float = 0.0,
    ) -> None:
        super().__init__(duration, delay)
        self.x, self.y = position
        self.color = color
    ...
    def draw(self, scene: Scene) -> None:
        ...
        for i in range(self.rings):
            ...
            scene.draw_image("ring", self.x - ring / 2, self.y - ring / 2, ring, ring, opacity=(1 - t) * 0.9, space="world", layer=RenderLayer.EFFECTS)
...
class Burst(Effect):
    ...
    def __init__(self, position: tuple[float, float], color: Color, count: int, *, rng: random.Random | None = None, delay: float = 0.0) -> None:
        super().__init__(self.LIFETIME[1] + 0.1, delay)
        self.count = count
        self.emitter = ParticleEmitter(
            "spark", position=position, speed=(60, 220), lifetime=self.LIFETIME, size=(14, 14), shrink=True,
