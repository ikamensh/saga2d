...
|---|---|---|
| W01 skirmish, difficulties, sizes | **evidence in place** | Three difficulties (`d4465fe`); 2–4 players on three sizes (`c8d0aec`); real-input match steps in `tools/verify_warband.py` (pass on the final code). `tools/ai_report.py --seeds 6 --decide 20 --ladder 4` on the final code (2026-09-06, Normal toned down so a plain opening has a chance): the scripted opening beat Easy 5/6, Normal 2/6, Hard 1/6; Normal-vs-Normal on Medium decided 19/20 within 20 minutes, mean 11.5 min. Retune 2026-09-09 (this commit; Easy first_wave 10→16, think_every 2.0→3.5, peasants 7→6, no visible-enemy counter shift; Normal peasants 9→6, think_every 1.5→2.0, first_wave 10→14, wave_growth 3→2, barracks 2→1, towers 1→0, reserve 1000→2000, keeps tech and repair; Hard unchanged): `tools/ai_report.py --seeds 6 --decide 4 --ladder 2` → script beat Easy 5/6, Normal 2/6, Hard 0/6; Normal beat Easy 3/4, Hard beat Normal 4/4 (Hard beat Easy 4/4); Normal-vs-Normal 4/4 decided within 20 minutes, mean 7.5 min. Ladder structure pinned by `test_difficulty_ladder_profiles_and_counter_shifts` in `tests/warband/test_ai.py`. Full report in the commit message. |
...
uv run --locked --isolated --python 3.13.2 --with-requirements packaging/requirements.txt python tools/build_game.py warband --version 0.1.0-preview.4  # W13; add --installer on Windows
uv run python tools/verify_game_package.py warband dist/warband --native
