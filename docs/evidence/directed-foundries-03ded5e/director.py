...
PATH = Path('/tmp/shardbound-manual-control-6648690.json.gz')
...
def inspect(state, request):
    ...
    out = dict(turn=state.turn, actions=state.actions_left, gold=state.gold,
               ...
               hero=dict(hp=hero.hp, max_hp=hero.max_hp, mana=hero.mana,
...
if not PATH.exists():
    source = ROOT / 'docs/evidence/shardbound-army-plans-cd351a9/control.json.gz'
    blob = source.read_bytes()
    assert hashlib.sha256(blob).hexdigest() == '6609eb4f334a956f6d6873e378b0b87bdd6901f3ad2f4688226265b8ede0b72a'
    ...
    data = dict(source=dict(path=str(source.relative_to(ROOT)), journal_sha256=hashlib.sha256(blob).hexdigest(),
                ...
                policy='Root-directed commands with explicit rationales. Historical opening used autoplay; '
