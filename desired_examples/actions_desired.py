...
def cast_fireball(caster, target_pos):
    fireball = Sprite(
        "sprites/fireball", position=caster.position, layer=RenderLayer.EFFECTS
    )
    fireball.do(
        Sequence(
            MoveTo(target_pos, speed=400),
            Do(
                lambda: ParticleEmitter(
                    "sprites/flame", position=target_pos, count=30
                ).burst()
            ),
...
def intro_cutscene():
    ...
    ).then(lambda: game.camera.pan_to(2000, 800, duration=2.0), 0.5).then(
        lambda: game.push(DialogScene("Knight", "I am ready.")), 1.0
    ).then(lambda: game.replace(GameWorldScene()), 0.0)
