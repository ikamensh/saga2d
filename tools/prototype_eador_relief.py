...
def passive_search(parties, *, trials=1000):
...
def source_audit():
    ...
    return dict(seeds_per_theme=1000,protected_sources=True,three_theme_relic_union=sorted(RELICS),
...
def measure():
    ...
    parties = {name:prepare(hero) for name,hero in (('commander','Commander'),('scout','Scout'))}
    for hero in HERO_CLASSES:
        party = Purchases(State.new(7,hero)); prepare_control_watch(party,kinds=())
    ...
    searches = passive_search(search_parties)
    ...
    return dict(source_revision=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
                ...
                plans=plans,automatic=automatic,passive_search=searches,source_audit=source_audit())


def interactive(variant):
    p = Orders(create(prepare('Scout' if variant=='scout' else 'Commander'), 'western' if variant=='western' else 'forward'))
    while True:
...
if __name__=='__main__':
    ...
    if args.interactive:interactive(args.interactive)
        ...
        report=measure();args.output.parent.mkdir(parents=True,exist_ok=True)
